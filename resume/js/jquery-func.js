...
function iniciarBinds(content,exp,pro,edu,blog,con,por,slider,menu,row_menu,statusSlider,box,statusSocial) {
    ...
    /**********active slider move box************************/
    /*jQuery('.resume_button ').click(function () {
     if (!statusSocial) {
     returnExperience(exp);
     returnProfile(pro);
     returnEducation(edu);
     returnBlog(blog);
     returnSocialContact(con);
     returnSocialPortfolio(por);
     statusSocial = hideSocialBox(box, menu, row_menu, status);
     if (statusSlider) {
     ...
     moveContact(con);
     movePortfolio(por);
     statusSlider = showSlider(slider, menu, row_menu, status);
     } else {
     ...
     returnContact(con);
     returnPortfolio(por);
     statusSlider = hideSlider(slider, menu, row_menu, status);
     }
     return false;
     } else {
     if (statusSlider) {
